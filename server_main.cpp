    ...
    timer.async_wait([&timer, interval_seconds](const boost::system::error_code& ec)
        {
            ...
            for (auto& s : sessions)
            {
                try {
                    ...
                    std::cerr << "[BATCH SAVE ERROR] " << e.what() << std::endl;
                }
            }
            ...
            std::cout << "[BATCH SAVE COMPLETE] Saved " << saved
                << " players in " << duration_ms << " ms." << std::endl;
...
int main()
{
    ...
    try {
        ...
        initialize_item_id_counter(*db_manager);
